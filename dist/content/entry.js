...
(() => {
  ...
  function registerTimestampEditService(helper) {
    ...
    const BRIDGE_SCRIPT_PATH3 = "dist/content/timestamp-bridge.js";
    ...
    helper.setSegmentBoundaryTime = async function setSegmentBoundaryTime(options) {
      ...
      for (let attempt = 0; attempt < attempts; attempt += 1) {
        ...
        const bridgeResult = await callTimestampBridge("set-boundary-time", {
          ...
          speakerKey: settings.speakerKey,
          targetSeconds,
          annotationId: rowIdentity && typeof rowIdentity.annotationId === "string" ? rowIdentity.annotationId : "",
          rowIdentity
        });
        if (bridgeResult && bridgeResult.ok) {
          return {
            ok: true,
            attempts: attempt + 1,
            backend: typeof bridgeResult.backend === "string" && bridgeResult.backend ? bridgeResult.backend : "page-react-row-time-change",
  ...
  var MIN_WAVEFORM_SCALE_VALUE = 0.3;
  var MAX_WAVEFORM_SCALE_VALUE = 1e3;
  ...
  function normalizeWaveformScaleValue(value) {
    const numeric = Number(value);
    if (!Number.isFinite(numeric)) {
      return null;
    }
  ...
  function normalizeWaveformScaleMap(source) {
    const incoming = source && typeof source === "object" && source !== null ? source : {};
    const normalized = {};
  ...
  function registerTimelineSelectionService(helper) {
    ...
    helper.commitCutPreview = async function commitCutPreview(options) {
      ...
      try {
        ...
        const duplicateSplitRows = initialOverlapPlan.splitRequired && splitRowSnapshot ? await helper.waitFor(
        ...
        const previousTrimLabels = shouldTrimPrevious && duplicateSplitRows && duplicateSplitRows.leftRow ? getRowTimeLabels(duplicateSplitRows.leftRow) : liveSnapshot.previous ? {
          startText: liveSnapshot.previous.startText,
          endText: liveSnapshot.previous.endText
        } : null;
        const nextTrimLabels = shouldTrimNext && duplicateSplitRows && duplicateSplitRows.rightRow ? getRowTimeLabels(duplicateSplitRows.rightRow) : liveSnapshot.next ? {
  ...
  function registerWaveformScaleService(helper) {
    ...
    function getWaveformScaleRows() {
      const sliders = Array.from(
        document.querySelectorAll('[role="slider"][data-orientation="vertical"]')
      );
    ...
    function formatInputValue(value) {
      ...
      return String(Math.round(numeric * 1e3) / 1e3);
    }
  ...
  function registerLifecycle(helper) {
    ...
    function bindSessionFeatures() {
      ...
      if (isFeatureEnabled("waveformScaleUnlock") && typeof helper.bindWaveformScaleUnlock === "function") {
