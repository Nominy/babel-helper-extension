...
(() => {
  ...
  function initMagnifierBridge() {
    ...
    const WAVEFORM_SCALE_DEFAULT_STEP = 1e-3;
    ...
    function getPreviewKeys(value) {
      ...
      return safe(() => Object.getOwnPropertyNames(value), []).filter(Boolean).slice(0, 14);
    }
    ...
    function isUsableWaveCandidate(value, host) {
      ...
      if (!(typeof value.setOptions === "function" || typeof value.zoom === "function" || typeof value.constructor?.create === "function")) {
        return false;
      }
    ...
    function describeReactPath(path, element) {
      ...
      const relatedFibers = fiber ? [{ fiber, path: baseFiberPath }].concat(getReactOwnerFibers(fiber, baseFiberPath)) : [];
      const describedFibers = relatedFibers.slice(0, 6).map((entry) => {
        const hooks = getReactHookValues(entry.fiber, entry.path).slice(0, 6).map((hookEntry) => ({
          path: hookEntry.path,
        ...
        return {
          ...
          typeName: safe(() => entry.fiber.type.displayName, null) || safe(() => entry.fiber.type.name, null) || safe(() => entry.fiber.elementType.displayName, null) || safe(() => entry.fiber.elementType.name, null) || null,
    ...
    function getSourceCanvasMetrics(wave) {
      ...
      const scope = container && container.shadowRoot ? container.shadowRoot : null;
      const canvas = scope && typeof scope.querySelector === "function" ? scope.querySelector('[part="canvases"] canvas') : null;
      const cssHeight = canvas instanceof HTMLCanvasElement ? parsePixels(canvas.style.height || "") || safe(() => canvas.getBoundingClientRect().height, 0) : 0;
      const intrinsicHeight = canvas instanceof HTMLCanvasElement && Number.isFinite(canvas.height) ? canvas.height : 0;
      return {
        cssHeight: cssHeight > 0 ? cssHeight : 0,
        intrinsicHeight: intrinsicHeight > 0 ? intrinsicHeight : 0,
        pixelRatio: cssHeight > 0 && intrinsicHeight > 0 ? intrinsicHeight / cssHeight : window.devicePixelRatio || 1
      };
    }
    function getSourceRenderMetrics(sourceWave, host) {
      ...
      const renderHeight = rendererHeight > 0 && rendererHeight || canvasMetrics.cssHeight > 0 && canvasMetrics.cssHeight || optionHeight > 0 && optionHeight || trackHeight > 0 && trackHeight || hostHeight > 0 && hostHeight || 1;
      return {
        renderHeight,
        ...
        pixelRatio: Number.isFinite(canvasMetrics.pixelRatio) && canvasMetrics.pixelRatio > 0 ? canvasMetrics.pixelRatio : 1,
    ...
    function buildLensOptions(sourceWave, mount, scale, sourceMetrics) {
      const options = sourceWave && sourceWave.options && typeof sourceWave.options === "object" ? Object.assign({}, sourceWave.options) : {};
      ...
      options.height = Number(sourceMetrics && sourceMetrics.renderHeight) > 0 ? Number(sourceMetrics.renderHeight) : Math.max(1, Number(safe(() => sourceWave.options.height, 0)) || 1);
    ...
    function syncLensDecodedAudio(record) {
      ...
      const decodedData = safe(() => record.sourceWave.getDecodedData(), null) || safe(() => record.sourceWave.decodedData, null) || safe(() => record.sourceWave.renderer.audioData, null);
      if (!decodedData) {
        return false;
      }
      safe(() => {
    ...
    function getSpeakerKeysForHost(host) {
      ...
      const trackProps = getTrackPropsForHost(host);
      const track = trackProps && typeof trackProps === "object" && trackProps.track && typeof trackProps.track === "object" ? trackProps.track : null;
      const keys = [];
    ...
    function resolveWaveForVisibleSpeaker(speakerKey) {
      ...
      if (normalizedSpeakerKey) {
        host = hosts.find((candidate) => getSpeakerKeysForHost(candidate).includes(normalizedSpeakerKey)) || null;
      }
    ...
    function getSourceRegionEntries(sourceWave) {
      ...
      const sourceRegions = safe(
        () => typeof regionPlugin.getRegions === "function" ? regionPlugin.getRegions() : Object.values(regionPlugin.regions || {}),
        []
      );
      return sourceRegions.map((region) => {
        ...
        return {
          ...
          backgroundColor: region && typeof region.color === "string" && region.color || styles && styles.backgroundColor || "rgba(176, 131, 255, 0.25)",
    ...
    function collectDiagnostics(host, mount, extra) {
      ...
      const diagnostics = {
        ...
        bestCandidate: best ? summarizeCandidateRecord(best, host) : null,
        fallbackCandidate: selection.fallback && selection.fallback !== best ? summarizeCandidateRecord(selection.fallback, host) : null,
        registryCandidates: collectRegistryCandidates(host).slice(0, 6).map((record) => summarizeCandidateRecord(record, host)),
    ...
    function getViewportMetrics(host, wave) {
      ...
      const visibleWidth = scroll instanceof HTMLElement ? Number(scroll.clientWidth) || 0 : wrapper instanceof HTMLElement ? Number(wrapper.clientWidth) || 0 : 0;
    ...
    function ensureLens(hostMarker, mountMarker, height, scale) {
      ...
      for (const [id2, record2] of instances.entries()) {
        if (record2.host === host && record2.mount === mount) {
      ...
      const factory = sourceWave && sourceWave.constructor && typeof sourceWave.constructor.create === "function" ? sourceWave.constructor.create.bind(sourceWave.constructor) : null;
    ...
    function findExactWaveCandidate(host) {
      ...
      const trackProps = getTrackPropsForHost(host);
      const trackId = trackProps && trackProps.track && typeof trackProps.track === "object" && trackProps.track.id != null ? String(trackProps.track.id) : null;
      const registryMatches = collectRegistryCandidates(host);
    ...
    function patchSliderNumericProps(slider, propName, nextValue) {
      ...
      while (node && typeof node === "object" && depth < 40) {
        ...
        if (memoizedProps && typeof memoizedProps === "object" && Number.isFinite(Number(memoizedProps[propName]))) {
          memoizedProps[propName] = nextValue;
          patched += 1;
        }
        const pendingProps = safe(() => node.pendingProps, null);
        if (pendingProps && typeof pendingProps === "object" && Number.isFinite(Number(pendingProps[propName]))) {
    ...
    function getWaveformScaleTrack(slider) {
      ...
      while (current instanceof HTMLElement) {
        if (current.getAttribute("data-orientation") === "vertical" && current !== slider && current.getBoundingClientRect().height > slider.getBoundingClientRect().height + 4) {
          return current;
        }
        current = current.parentElement;
      }
    ...
    function getWaveformScaleVisualParts(slider) {
      ...
      const track = root instanceof HTMLElement ? root.querySelector(":scope > span:first-child") : null;
      const range = track instanceof HTMLElement ? track.querySelector(":scope > span") : null;
      const tooltip = slider instanceof HTMLElement ? slider.querySelector("div") : null;
      return {
    ...
    function patchWaveformScaleSlider(slider) {
      ...
      const nextMax = Math.max(WAVEFORM_SCALE_DEFAULT_MAX, Number(waveformScaleState.max) || 1e3);
    ...
    function getWaveformScaleSliderByIndex(index) {
      ...
      const sliders = Array.from(document.querySelectorAll(WAVEFORM_SCALE_SELECTOR)).filter(
        (slider) => isWaveformScaleSlider(slider)
      );
      const target = sliders[numericIndex];
      return target instanceof HTMLElement ? target : null;
    }
    function getWaveformScaleControl(target) {
      const slider = target instanceof HTMLElement ? target.closest(WAVEFORM_SCALE_SELECTOR) : null;
      ...
      return {
        ...
        max: Math.max(
          Number(waveformScaleState.max) || 1e3,
...
//# sourceMappingURL=magnifier-bridge.js.map
