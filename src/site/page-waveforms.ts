...
export function initMagnifierBridge() {
  ...
  function waitForZoomReady(timeoutMs) {
    ...
    function checkReady() {
      ...
      // Native zoom applies once the visible track can zoom. Collapsed or
      // hidden tracks (and tracks that never decode) must not hold the saved
      // default back; a wave without a known container counts as visible.
      // Duration and rendered canvases can exist before WaveSurfer can zoom.
      const records = collectRegistryCandidates(slider);
      let visibleCount = 0;
      let visibleDecoded = true;
      let anyDecoded = false;
      for (const { value: wave } of records) {
        const decoded =
          typeof wave.getDecodedData === 'function' && Boolean(safe(() => wave.getDecodedData(), null));
        const container = getCandidateContainer(wave);
        if (!container || isVisibleElement(container)) {
          visibleCount += 1;
          visibleDecoded = visibleDecoded && decoded;
        }
        if (decoded) {
          anyDecoded = true;
          continue;
      ...
      if (visibleCount ? visibleDecoded : anyDecoded) finish({ ok: true });
    }
