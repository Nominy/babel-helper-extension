...
- On a Babel dashboard page, use the green `🌿` toolbar button or press `Alt + Shift + P` to open or close the on-page live editor. The button shares the dashboard toolbar with Gold Drafting's magic wand when that extension is installed. The shortcut also works while focus is inside the editor; press `Escape` or use Close to dismiss it.
